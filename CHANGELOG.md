...
- Raise the file size limit to 500 MB and enforce it in one place. `Yust.maxFileSizeInBytes` is the new hard ceiling for every upload and download; `downloadFile`'s `maxSize` default rises from 20 MB to it, and `uploadFile`/`uploadStream` now reject anything larger via `Yust.validateFileSize`. Callers may still apply a stricter limit of their own.
